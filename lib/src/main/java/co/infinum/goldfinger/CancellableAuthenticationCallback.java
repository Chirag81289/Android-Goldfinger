...
class CancellableAuthenticationCallback extends FingerprintManagerCompat.AuthenticationCallback {
    ...
    private static final long IGNORE_CANCEL_MS = 150;
    ...
    public void onAuthenticationError(int errMsgId, CharSequence errString) {
        ...
        if (!cancellationSignal.isCanceled() && shouldReactToError(error)) {
    ...
    private boolean shouldReactToError(Error error) {
        return error != Error.CANCELED
                || clock.isBeforeNow(initializationTime + IGNORE_CANCEL_MS);
    }
}
